...
# ##### 1) Logistic regression classifier for speeches from the Senate;
# In[1]:
import os
import sys
...
# returns two lists (of the same length): the cleaned lines and the party of the
person who was speaking
#
# Make sure to replace line-internal whitespace (newlines, tabs, etc.) in text with
a space.
#
...
# Call sklearn's train_test_split function to split the dataset into training
items/labels
...
#
# Note that the train_test_split function returns four sequences: X_train, X_test,
y_train, y_test
...
#    analyzefn - either built-in (see CountVectorizer documentation), or a function
we provide from strings to feature-lists
#
#
#
#
#
(1,2) for unigrams and bigrams
#
Arguments used by the words analyzer
  ...
  ngram_range - (N,M) for using ngrams of sizes N up to M as features, e.g.

# Outputs:
#     X_features - corresponding feature vector for each raw text item in X
#     training_vectorizer - vectorizer object that can now be applied to some new
X', e.g. containing test texts
#
# You can find documentation at
https://scikit-learn.org/stable/modules/generated/sklearn.feature_extraction.text.C
ountVectorizer.html
# and there's a nice, readable discussion at https://medium.com/swlh/understanding-
count-vectorizer-5dd71530c1b
#
...
#
# Output:  a corresponding list converting the raw strings to space-separated
features
#
# The features extracted should include non-stopword, non-punctuation unigrams,
# plus the bigram features that were counted in collect_bigram_counts from the
previous assignment
...
#
#
def convert_lines_to_feature_strings(lines, stopwords,
remove_stopword_bigrams=True):
    ...
    for line in tqdm(lines):
        ...
        # Exclude unigrams that are stopwords or are punctuation strings (e.g. '.'
or ',')
...
bigrams
if remove_stopword_bigrams:
= ["_".join(bigram) for bigram in bigrams]
= ngrams(normalized_tokens, 2)
= filter_punctuation_bigrams(bigrams)
            bigrams = filter_stopword_bigrams(bigrams, stopwords)
        bigram_tokens = ["_".join(bigram) for bigram in bigrams]
        # Conjoin the feature lists and turn into a space-separated string of
features.
        # E.g. if unigrams is ['coffee', 'cup'] and bigrams is ['coffee_cup',
'white_house']
...
def most_informative_features(vectorizer, classifier, n=20):
    # Adapted from https://stackoverflow.com/questions/11116697/how-to-get-most-
informative-features-for-scikit-learn-classifiers#11116960
    feature_names       = vectorizer.get_feature_names()
    coefs_with_features = sorted(zip(classifier.coef_[0], feature_names))
    top                 = zip(coefs_with_features[:n], coefs_with_features[:-(n +
1):-1])
    for (coef_1, feature_1), (coef_2, feature_2) in top:
        print("\t%.4f\t%-15s\t\t%.4f\t%-15s" % (coef_1, feature_1, coef_2,
feature_2))
...
def main(use_sklearn_feature_extraction, num_most_informative, plot_metrics):
    ...
    # Read the dataset in and split it into training documents/labels (X) and test
documents/labels (y)
    X_train, X_test, y_train, y_test =
split_training_set(*read_and_clean_lines(input_speechfile))
    if use_sklearn_feature_extraction:
        # Use sklearn CountVectorizer's built-in tokenization to get unigrams and
bigrams as features
        X_features_train, training_vectorizer = convert_text_into_features(X_train,
stop_words, "word", range=(1,2))
        ...
        X_train_feature_strings = convert_lines_to_feature_strings(X_train,
stop_words)
        print("Creating feature strings for test data")
        X_test_documents        = convert_lines_to_feature_strings(X_test,
stop_words)
        # Call CountVectorizer with whitespace-based tokenization as the analyzer,
so that it uses exactly your features,
        # but without doing any of its own analysis/feature-extraction.
        X_features_train, training_vectorizer =
convert_text_into_features(X_train_feature_strings, stop_words,
whitespace_tokenizer)
    # Create a logistic regression classifier trained on the featurized training
data
    ...
    most_informative_features(training_vectorizer, lr_classifier,
num_most_informative)
    # Apply the "vectorizer" created using the training data to the test documents,
to create testset feature vectors
    X_test_features =  training_vectorizer.transform(X_test_documents)
    # Classify the test data and see how well you perform
    # For various evaluation scores see
https://scikit-learn.org/stable/modules/model_evaluation.html
    print("Classifying test data")
    predicted_labels = lr_classifier.predict(X_test_features)
    print('Accuracy  = {}'.format(metrics.accuracy_score(predicted_labels,
y_test)))
...
metrics.recall_score(predicted_labels,    y_test, pos_label=label)))
    ...
    metrics.plot_confusion_matrix(lr_classifier, X_test_features, y_test,
normalize='true')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--use_sklearn_features', default=False,
action='store_true', help="Use sklearn's feature extraction")
    parser.add_argument('--plot_metrics', default=False, action='store_true',
help="Generate figures for evaluation")

    parser.add_argument('--num_most_informative', default=10, action='store',
help="Number of most-informative features to show")
    args, unknown = parser.parse_known_args()
    main(args.use_sklearn_features, int(args.num_most_informative),
args.plot_metrics)
# ##### 2) Logistic regression classifier for speeches from the House;
# In[2]:
import os
import sys
from sklearn.linear_model import LogisticRegression
...
help="Number of most-informative features to show")
    ...
    main(args.use_sklearn_features, int(args.num_most_informative),
args.plot_metrics)
# ##### 3) Logistic regression classifier for speeches from both House and Senate
combine;
# In[3]:
import os
import sys
...
print("\nReading and cleaning text from {}".format(infile))
    ...
    with gzip.open(infile,'rt') as f:
        for line in tqdm(f):
          data = json.loads(line)
          if data['chamber'] == "House" or data['chamber'] == "Senate":
              txt = data['text']
              txt = " ".join(txt.split())
              lines.append(txt)
              parties.append(data['party'])
